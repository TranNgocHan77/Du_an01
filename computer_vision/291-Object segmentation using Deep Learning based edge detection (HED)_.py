...
12. Export your data​
==================================================================================================
Mô hình Caffe được mã hóa thành hai tệp
1. Tệp văn bản proto: https://github.com/s9xie/hed/blob/master/examples/hed/deploy.prototxt
2. Mô hình caffe sơ chế: http://vcl.ucsd.edu/hed/hed_pretrain_bsds.caffemodel
LƯU Ý: Trong tương lai, nếu các liên kết này không hoạt động, tôi không thể trợ giúp. Vui lòng Google
và tìm các liên kết cập nhật (thông tin cập nhật tính đến tháng 10 năm 2022)

Các bước để phát hiện cạnh, sau đó là ghi nhãn dựa trên các thành phần được kết nối
để phân đoạn đối tượng:
    
1. Xác định lớp cắt (không được triển khai theo mặc định) ​
2. Xác định mạng và tải mô hình được đào tạo trước.​
3. Đăng ký lớp cắt với mạng
4. Tạo blob từ hình ảnh – về cơ bản là tạo một hình ảnh được xử lý trước​
5. Tải mô hình đã được huấn luyện trước (bạn cần cả tệp văn bản gốc và tệp mô hình caffe)​
6. Truyền hình ảnh blob qua mô hình​ (chuyển tiếp)
7. Nhận đầu ra​
8. Chuẩn bị sẵn hình ảnh cho các thành phần được kết nối (làm mờ, ngưỡng)​
9. Thực hiện ghi nhãn dựa trên các thành phần được kết nối​
10. (Tùy chọn) vẽ điểm đánh dấu cho mục đích trực quan​
11. (Tùy chọn) lọc các vật thể nhỏ​
12. Xuất dữ liệu của bạn​
...
# Có một lớp Cắt mà mạng HED sử dụng không được triển khai bởi
# mặc định vì vậy chúng tôi cần cung cấp cách triển khai riêng cho lớp này.
#Không có lớp cắt, kết quả cuối cùng sẽ bị dịch chuyển sang bên phải và phía dưới
...
class CropLayer(object):
    def __init__(self, params, blobs):
        ...
        # khởi tạo tọa độ bắt đầu và kết thúc (x, y) của
        # vụ mùa
        self.startX = 0
        self.startY = 0
        self.endX = 0
    ...
    def getMemoryShapes(self, inputs):
        ...
        # lớp cắt sẽ nhận được hai đầu vào -- chúng ta cần cắt
        # đốm màu đầu vào đầu tiên khớp với hình dạng của đốm màu thứ hai,
        # giữ kích thước lô và số lượng kênh
        ...
        # tính tọa độ cắt bắt đầu và kết thúc
        ...
        # trả về hình dạng của tập đĩa (chúng ta sẽ thực hiện việc thực tế
        # cắt trong quá trình chuyển tiếp
        return [[batchSize, numChannels, H, W]]

    def forward(self, inputs):
        # use the derived (x, y)-coordinates to perform the crop

        # sử dụng tọa độ dẫn xuất (x, y) để thực hiện cắt xén
...
# đăng ký lớp cắt của chúng tôi với mô hình
cv2.dnn_registerLayer("Crop", CropLayer)

#================================================================================================

# load the input image and grab its dimensions, for future use while defining the blob
# tải hình ảnh đầu vào và lấy kích thước của nó để sử dụng sau này trong khi xác định blob
...
# tạo một đốm màu từ hình ảnh đầu vào
#blob về cơ bản là hình ảnh được xử lý trước. Mô-đun mạng thần kinh sâu (dnn ) mới của #OpenCV chứa hai hàm
#có thể được sử dụng để xử lý trước hình ảnh và chuẩn bị chúng cho
#phân loại thông qua các mô hình học sâu được đào tạo trước.
# Nó bao gồm chia tỷ lệ và phép trừ trung bình
...
#Xem hình ảnh sau khi tiền xử lý (blob)
...
# đặt blob làm đầu vào cho mạng và thực hiện chuyển tiếp
# để tính các cạnh
...
#Ghi nhãn dựa trên thành phần được kết nối

# Load segmented binary image, Gaussian blur, grayscale, Otsu's threshold
# Tải hình ảnh nhị phân được phân đoạn, độ mờ Gaussian, thang độ xám, ngưỡng của Otsu
...
# Thực hiện ghi nhãn thành phần được kết nối
...
# Tạo ảnh sai màu với nền đen và các vật thể có màu
